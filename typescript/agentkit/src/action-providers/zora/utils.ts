 ...
 * @returns A promise that resolves to the IPFS URI for the metadata
 */
export async function generateZoraTokenUri(params: TokenUriParams): Promise<string> {
  try {
    ...
    const ipfsImageUri = `ipfs://${imageRes.IpfsHash}`;
